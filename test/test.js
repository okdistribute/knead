...
test('test from checkouts', function (t) {
  var db = dat(memdb(), {valueEncoding: 'json'})
  createTableConflicts(db, function (branches) {
    var stream1 = db.checkout(branches[0]).createReadStream()
    var stream2 = db.checkout(branches[1]).createReadStream()
    var opts = {
      html: false,
      limit: 30
    }
    dat2daff.fromReadStreams(stream1, stream2, opts, function (err, visual, next) {
      console.log(visual)
      t.end()
    })
  })
