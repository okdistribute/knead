 ...
 *
 */
function NdjsonTable (data,mapping) {
    this.data = data;
...
NdjsonTable.prototype.getCell = function(x,y) {
    ...
    if (y==0) return key;
    var mapping = this.mapping(this.data[y-1])
    if (mapping && mapping.hasOwnProperty(key)) {
        console.log('mapping', key, mapping, x, y)
        return mapping[key]
    }
    else {
        console.log('null', key, mapping, x, y)
        return null
    }
}
