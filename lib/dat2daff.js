...
var dat2daff = function (stream1, stream2, opts, cb) {
  ...
  function createBatchedStream (stream) {
    var batchedStream = batcher(opts.limit)
    return stream.pipe(batchedStream)
  }

  stream1 = createBatchedStream(stream1)
  stream2 = createBatchedStream(stream2)

  dat2daff.batchedStreamToTable(stream1, function (table1) {
    dat2daff.batchedStreamToTable(stream2, function (table2) {
